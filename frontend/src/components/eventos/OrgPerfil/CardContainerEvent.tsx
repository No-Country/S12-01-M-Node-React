...
interface CardContainerEventProps {
  eventos: Eventos[];
  currentIndex: number;
  index?: boolean;
...
export const CardContainerEvent: React.FC<CardContainerEventProps> = ({
  eventos,
  currentIndex,
  index,
  setIndex
  ...
  return (
    <article className="flex gap-4 overflow-hidden">
      {eventos.map((evento, i) => (
        <div
          key={evento._id}
          className="h-[400px] bg-gradient-to-tr from-pink-600 via-indigo-500 to-indigo-500 rounded-[20px] p-[1px] basis-[331px] grow-0 shrink-0 inline-block relative"
          style={{
